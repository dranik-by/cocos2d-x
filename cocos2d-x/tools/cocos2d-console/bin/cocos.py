...
class Cocos2dIniParser:
    def __init__(self):
        ...
        print("self.cocos2d_path", self.cocos2d_path)
...
def _check_dependencies_exist(dependencies, classes, plugin_name):
    print("_check_dependencies_exist")
...
def copy_files_with_config(config, src_root, dst_root):
    ...
    print("include_rules", include_rules)
    print("exclude_rules", exclude_rules)
...
if __name__ == "__main__":
    ...
    print("ver_pattern", ver_pattern)
    print("COCOS_ENGINE_VERSION", COCOS_ENGINE_VERSION)

    match = re.match(ver_pattern, COCOS_ENGINE_VERSION)
    if match:
        STAT_VERSION = match.group(1)
    print("STAT_VERSION", STAT_VERSION)
